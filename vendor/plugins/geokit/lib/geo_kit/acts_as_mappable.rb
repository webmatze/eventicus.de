    ...
    module InstanceMethods #:nodoc:    
      ...
      module SingletonMethods # :nodoc:
        ...
        def apply_distance_scope(options)
          distance_condition = "#{distance_column_name} <= #{options[:within]} OR #{distance_column_name} IS NULL" if options.has_key?(:within)
        ...
        # Wrapper around adapter fetching to enable seamless operation with
        # JDBC/jruby
        def get_connection_adapter
          adapter = connection.adapter_name.downcase
          adapter = connection.config[:adapter].downcase if adapter == 'jdbc'
          adapter
        end
